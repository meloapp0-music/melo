...
- `2026-07-16-notification-digest.md` — built (dev) · **Three phases.** Digest bundles ambient discovery into one daily push. Then "first to know": `presale-sweep` (*/15) queries per CITY not per artist — scaling with cities, not artists — and writes every presale's start time into `presale_schedule` (migration 0022); `presale-fire` (every minute) reads that clock and pushes "presale is live" with **zero API calls**, because TM already told us when. Replaced the hourly `presale-watch` written hours earlier, which couldn't be fast enough by construction. 3,400 of 5,000 daily TM calls. Nothing deployed. The three discovery kinds stop firing one push each (up to five back-to-back) and leave as one daily digest; `preshow_*`/`postshow_rate` stay individual because they're personal and time-critical. New hourly `presale-watch` function handles the urgent half — it inverts watch sets into `artist → users` before querying, so an artist 500 people watch costs one Ticketmaster call, which is the only reason an hourly cadence fits the API budget. Neither deployed.
- `2026-08-03-the-room.md` — planned · Who else was at this show. The biggest idea from the strategy pass and the only one with neither code nor design; stalled twice on the same blocker, that `shows` has no event identity. Specs a derived `event_key` that works on existing rows today, with a canonical `event_id` layered on later, plus the privacy rule (the room never reveals a row the viewer couldn't already see) and a count that reads past RLS but returns only a number.
- `2026-08-03-artist-palette.md` — built (dev) · Eight curated artist colours replacing a hue hash that was locked to 8–44°, so every artist in the app was a shade of the same orange. Also splits Home's Up Next hero into colour-under-photo layers, and consolidates seven copies of the photo-over-gradient pattern — fixing a latent blank-card bug on four of them.
...
- `2026-07-16-notification-digest.md` — planned · Kill the daily "notification after notification" storm (the alert cron sends up to 5 separate pushes/user in one noon run). **Phase 1**: bundle the discovery alerts (tour/city/genre) into ONE daily digest push → a "For You" page (recommendations first + see-all), reusing the Tonight/Discover surface; keep preshow/postshow individual. **Phase 2**: a real-time presale/just-dropped watcher (more frequent poll + TM `sales.presales` data) for the "know the second it drops" urgency. Cron + web-only → ships with no iOS build.
- `2026-07-16-tonight-in-your-city.md` — built (dev) · A "Tonight in {city}" rail on Discover — what's playing in your home city TODAY, auto-loaded (no search), taste-first, tap→tickets. Home city inferred from logged shows (no GPS/setting); rolls over at local midnight via `dayStamp`. Verified against real TM data (9 real Chicago shows). Born from Aidan wanting a daily-content generator — the honest pushback was that his own marketing OS says ideas aren't the bottleneck, execution is; he chose the in-app feature over a cron→email, trading automation for product value (he still has to screenshot it).
- `2026-07-15-know-before-you-go.md` — built + reviewed (5 fixes) + live-verified (dev) · Show-day "Know Before You Go" card (day-of pop-up + ShowDetail section, shared `ShowDayInfo`). Deep-links artist/venue Instagram + surfaces real Ticketmaster show-day info (showtime, rules, parking, box office) with zero extra requests. Honest about the limit — can't fetch the actual IG post (no API), so it lands you on the profile via a verified Wikidata handle or a site-scoped search. Verified live on Noah Kahan @ Wrigley + Goose @ Red Rocks.
- `2026-07-15-stats-year-filter.md` — built + verified (dev) · Year filter on the Stats page — `[All Time] [2026] [2025]…` chip row (shown when ≥2 years); one `scoped` filter re-scopes every stat. Designed by a 3-lens panel: a specific year drops the today-anchored "current streak" (shows longest-within-year, gated ≥2); tiles that jump to all-time lists go inert in year scope while entity rows stay live (Most Seen rerouted to ArtistDetail). Default All Time = no regression.
- `2026-07-14-artist-image-disambiguation.md` — shipped (dev) · Fixed wrong artist photos for shared names — "Goose" showed the Belgian dance band, not the CT jam band, because the Deezer resolver took the most-popular hit blindly. Now disambiguates by matching each same-name candidate's catalog against the user's own logged setlist (10 song matches for the jam band vs 0 for the Belgian one). Image cache bumped to v2 to flush the poisoned entries; unambiguous artists skip it at no cost.
- `2026-07-13-stat-collection-pages.md` — in-progress · Turning the Stats-linked detail pages into personality-driven "collections" like Your Rooms. Shipped **Your Lineup** (Artists: photo-wall grid + #1-seen hero + Most seen/By genre/Recently toggles + a new ArtistDetail page), festival consolidation + festival-level media, the **Map** travel band ("N cities · N states · N miles") + interactive city rail that cross-highlights with the pins, and **"Your Anthems"** on Songs (top-5 heard-more-than-once quick-play strip).
- `2026-07-13-home-declutter.md` — in-progress · Home felt "hectic" in live testing. Wrapped banner + streak off the stat bar; then a bigger restructure the user chose: **Buddies tab → a new Stats tab**, feed-first Home (Up Next → Friends feed), discovery trimmed 5 sections → 2, and a friends-feed merge so a friend going to a show you're also going to reads "You + Claire are going" instead of repeating your own Up Next. Header-gradient seam fixed.
- `2026-07-12-v1.5-release-prep.md` — build prepped, awaiting device test · v1.5 / build 34 — bundles the festival overhaul + tour browser + music-taste/notification work that all landed AFTER the v1.4 submission. Prompted by a real failure (the user's sisters couldn't log Windy City Smokeout because every festival fix sits in commits after the v1.4 tag). Includes the TestFlight device-test checklist — the one verification step never done.
- `2026-07-13-venue-photos.md` — in-progress · Real venue photos on "Your Rooms" from Wikimedia Commons (Wikipedia pageimages API — keyless, CORS, ~90% coverage for arenas/amphitheatres/festival grounds; small clubs degrade to the gradient). Ticketmaster rejected as a source (generic stock, ToS forbids caching). Waterfall = Wikipedia pageimages → Wikidata P18 → gradient, guarded by a coordinate + name-token false-match check; mirrors the existing artist-image resolver pattern. Source chosen via a 5-agent research workflow live-tested against real venues.
- `2026-07-10-wishlist-search-consolidation.md` — shipped · Collapsed Going/Wishlist logging from 3 mode tabs to 2 (Search / Festival); Search = artist lookup with the full tour + a manual fallback for shows in no DB; genre autofills from the event's TM classification. Also removed the redundant inline Festival field from the quick-log form. A 4-lens adversarial workflow caught a MAJOR wishlist-edit regression + 4 minors — all fixed and re-verified.
- `2026-07-09-festival-search-and-notification-deeplink.md` — shipped · two real user reports: (1) festival search extended to Wishlist/Going (verified live against Ticketmaster's real Windy City Smokeout listing, fixed a duplicate-event bug where the festival's own name appeared as a fake headliner); (2) notification taps went nowhere — found `genre_alert` was never added to the deep-link handler, and upgraded the destination from a vanishing toast to a real pre-searched Full Tour screen.
- `2026-07-09-artist-tour-browser.md` — shipped · real user feedback: searching an artist should show their whole upcoming tour (any city), not just the top 8 nearby. New "Full Tour" mode on Going/Wishlist — multi-select, reuses the Festival-picker plumbing (`fetchUpcomingEventsMulti`, no city filter). Caught + fixed a mode-state bug in review (switching status left the wrong tab-set unhighlighted) and a hardcoded-status bug in the shared submit function.
- `2026-07-07-music-taste-discoverability.md` — shipped · Music Taste (genres/artists/city) was 3 taps deep in Settings despite now driving real notification behavior. Promoted to its own Profile-level page (`MusicTaste.jsx`); Settings keeps a link-row to it. Added a one-time Home nudge (`TasteNudge.jsx`) for users who got past GetStarted's onboarding checklist without ever setting taste — fills the exact gap GetStarted's own dismissed-forever card can't cover.
- `2026-07-05-genre-based-notifications.md` — shipped + deployed (v16 live) · Settings' "favorite genres" did nothing for notifications — tour-alerts only ever watched named artists. Now genre picks power their own "any artist in this genre, in my city" alerts via a round-robin allocator (shuffled genre order, one pick per genre per pass) so a busy genre can't drown out a quiet one. Caught + fixed a dedup bug in review (genre_alert wasn't in the prior-sends filter).
- `2026-07-05-ux-papercuts-sweep.md` — shipped · a "walk the app like a picky user" audit (6-agent, code-verified) found 20 small UX gaps — dead-end taps, missing empty/loading states, no-feedback actions — across Home/My Shows/Show Detail/Log Show/Buddies/Concert Map/Festivals. All 20 fixed in one pass: toasts on Wishlist/empty-artist-submit, save-button pending states, a real in-app delete-confirm sheet, Concert Map now resolves every city (was a stale 34-city list), photo removal on Show Detail, filter chips that actually clear, and more.
- `2026-07-01-artist-tracking-anywhere.md` — in-progress (research) · fix the coverage hole: search + alerts are Ticketmaster-only so small-venue shows (Congress @ Schubas) are invisible; goal = any-band-anywhere search + genre×city watch alerts. Research workflow running on which 2026 APIs actually cover Schubas-tier venues.
- `2026-06-27-video-uploads.md` — in-progress · **v1 BUILT + migration 0014 APPLIED (2026-07-02)**: shows.videos + show-videos bucket live (RLS mirrors photos + 45MB/MIME caps), VideoPicker (≤3 clips, ≤60s validated client-side), LogShow Videos section, ShowDetail playback. Remaining: device test. Montage/recap = phase 2. (Also fixed CLI migration-history drift: repaired 0006–0013 as applied.)
- `2026-06-29-ticket-stub-variants.md` — in-progress · lean the share card into the ticket stub: build multiple ticket ARCHETYPES (classic stub / boarding pass / receipt / VIP laminate / festival band) + a full-setlist-on-the-image variant (the receipt), and trim Poster + Marquee (keep Vibe/Player). Decided 2026-06-29; 5 mockups presented for selection. The "viewers expand the setlist" half = public-share-pages (later).
...
- `2026-06-23-public-share-pages.md` — in-progress · **Phase 0+1 built (dev)** · `melo.show/s/<token>` — a server-rendered public show page (Cloudflare Pages Function) so a shared card is finally clickable: full setlist + photos + videos + install CTA, for someone without the app. Opt-in `share_token` + a SECURITY DEFINER RPC (migration 0016) — strictly additive, `shows` RLS untouched. The card's QR now points at the SHOW, not the App Store. `og:video` makes **iMessage autoplay a clip in the link bubble** — "video on the share card" with zero encoding. Videos are tap-to-play (the ~$600-vs-$9,000-at-scale call). Also fixed a live lie: the app promised "60s max" the 45MB cap could never allow (45MB ≈ 16s at 4K — the cap is the Supabase FREE tier's 50MB ceiling showing through). **Needs: migration 0016 applied · Cloudflare env vars · `wrangler pages deploy` (drag-drop can't compile /functions).**
...
- `2026-06-16-preship-qa-pass.md` — shipped (next build) · 6-agent
  ...
  EULA at sign-up, a cluster of safe-area/nav-bar layout bugs, legal
  draft banners + UGC clause, unblock UI, and a find-friends prompt.
