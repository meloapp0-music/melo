...
- `2026-07-24-nfl-schedule-predictor.md` — shipped · standalone, self-contained `public/nfl-2026-predictor.html` — an NFL 2026-27 pick 'em sheet (real 272-game schedule, per-team and per-week pick lanes with keyboard picking, real NFL tiebreakers, playoff bracket, head-to-head compare against a friend's link, receipts screen with a shareable card, URL-encoded share links). A personal one-off for the user to send to friends; touches no Melo code or data.
