...
- `2026-07-05-genre-based-notifications.md` — shipped (code), **not yet deployed** · Settings' "favorite genres" did nothing for notifications — tour-alerts only ever watched named artists. Now genre picks power their own "any artist in this genre, in my city" alerts via a round-robin allocator (shuffled genre order, one pick per genre per pass) so a busy genre can't drown out a quiet one. Caught + fixed a dedup bug in review (genre_alert wasn't in the prior-sends filter). Awaiting go-ahead to `supabase functions deploy tour-alerts`.
